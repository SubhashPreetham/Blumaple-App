...
type ReturnScreen = 'home' | 'categories' | 'categoryCollection' | 'wishlist' | 'offers' | 'orders' | 'search' | 'discoverySearch' | 'homeProducts' | 'product' | 'cart';
...
function ProductCard({ item, width, favorite, quantity = 0, onFavorite, onAdd, onChangeQuantity, onOpen, showFavorite = true, collectionLayout = false }: { item: Product; width: number; favorite: boolean; quantity?: number; onFavorite: () => void; onAdd: () => void; onChangeQuantity?: (change: number) => void; onOpen?: () => void; showFavorite?: boolean; collectionLayout?: boolean }) {
  ...
  if (collectionLayout) {
    ...
    return <View style={[styles.collectionProductCard, { width }]}>
      ...
      <View style={styles.collectionUnitRow}>{availableForSale && quantity > 0 ? <View style={[styles.collectionImageAction, styles.collectionQuantityAction]}><Pressable hitSlop={8} onPress={() => onChangeQuantity?.(-1)} style={styles.cardQuantityTap}><Ionicons name="remove" size={13} color="#FFFFFF" /></Pressable><Text style={styles.cardQuantityText}>{quantity}</Text><Pressable hitSlop={8} onPress={() => onChangeQuantity?.(1)} style={styles.cardQuantityTap}><Ionicons name="add" size={13} color="#FFFFFF" /></Pressable></View> : <Pressable onPress={availableForSale ? onAdd : notifyConfirmation.notify} style={({ pressed }) => [styles.collectionImageAction, !availableForSale && styles.collectionNotifyAction, !availableForSale && notifyConfirmation.notified && styles.collectionNotifiedAction, pressed && styles.pressed]}>{availableForSale ? <Text style={styles.collectionImageActionText}>ADD</Text> : <NotifyConfirmation notified={notifyConfirmation.notified} showMessage={notifyConfirmation.showMessage} color="#2E8B36" />}</Pressable>}</View>
  ...
  return (
    <View style={[styles.productCard, { width }]}>
      ...
      {quantity > 0 ? <View style={[styles.cartButton, styles.productCardQuantityButton]}><Pressable hitSlop={8} onPress={() => onChangeQuantity?.(-1)} style={styles.cardQuantityTap}><Ionicons name="remove" size={16} color="#FFFFFF" /></Pressable><Text style={styles.cardQuantityText}>{quantity}</Text><Pressable hitSlop={8} onPress={() => onChangeQuantity?.(1)} style={styles.cardQuantityTap}><Ionicons name="add" size={16} color="#FFFFFF" /></Pressable></View> : <Pressable accessibilityRole="button" onPress={onAdd} style={({ pressed }) => [styles.cartButton, pressed && styles.pressed]}><Text style={styles.cartButtonText}>Add to cart</Text></Pressable>}
    </View>
  );
}
...
function HomeProductListingPage({ title, products, favoriteIds, cartItems, cartCount, onBack, onFavorite, onAdd, onChangeCartQuantity, onOpen, onOpenSearch, onOpenCart, onOpenWishlist }: { title: string; products: Product[]; favoriteIds: Set<string>; cartItems: CartItem[]; cartCount: number; onBack: () => void; onFavorite: (product: Product) => void; onAdd: (product: Product) => void; onChangeCartQuantity: (productId: string, change: number) => void; onOpen: (product: Product) => void; onOpenSearch: () => void; onOpenCart: () => void; onOpenWishlist: () => void }) {
  ...
  return <View style={styles.homeListingPage}>
    <View style={styles.homeListingHeader}><Pressable onPress={onBack} hitSlop={10} style={styles.homeListingBack}><Ionicons name="arrow-back" size={24} color={palette.white} /></Pressable><View style={styles.homeListingHeaderCopyBlock}><Text style={styles.homeListingHeaderTitle}>{title}</Text><Text style={styles.homeListingHeaderCopy}>{visibleProducts.length} products</Text></View><View style={styles.homeListingHeaderActions}><Pressable accessibilityRole="button" accessibilityLabel="Open search" onPress={onOpenSearch} style={styles.homeListingHeaderAction}><Ionicons name="search-outline" size={24} color={palette.white} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel={`Open wishlist, ${favoriteIds.size} products`} onPress={onOpenWishlist} style={styles.homeListingHeaderAction}><Ionicons name={favoriteIds.size ? 'heart' : 'heart-outline'} size={23} color={palette.white} />{favoriteIds.size ? <View style={styles.homeListingBadge}><Text style={styles.homeListingBadgeText}>{favoriteIds.size > 99 ? '99+' : favoriteIds.size}</Text></View> : null}</Pressable><Pressable accessibilityRole="button" accessibilityLabel={`Open cart, ${cartCount} items`} onPress={onOpenCart} style={styles.homeListingHeaderAction}><Ionicons name="bag-handle-outline" size={23} color={palette.white} />{cartCount ? <View style={styles.homeListingBadge}><Text style={styles.homeListingBadgeText}>{cartCount > 99 ? '99+' : cartCount}</Text></View> : null}</Pressable></View></View>
    <View style={styles.homeListingBody}>
      {visibleProducts.length ? <ScrollView showsVerticalScrollIndicator={false} bounces alwaysBounceVertical decelerationRate="normal" scrollEventThrottle={16} contentContainerStyle={styles.homeListingProductScroll}><View style={styles.homeListingProductGrid}>{visibleProducts.map(product => <ProductCard key={`${title}-${product.id}`} item={product} width={cardWidth} favorite={favoriteIds.has(product.id)} quantity={cartItems.find(item => item.product.id === product.id)?.quantity ?? 0} collectionLayout onFavorite={() => onFavorite(product)} onAdd={() => onAdd(product)} onChangeQuantity={change => onChangeCartQuantity(product.id, change)} onOpen={() => onOpen(product)} />)}</View></ScrollView> : <View style={styles.homeListingEmpty}><Ionicons name="cube-outline" size={45} color={palette.blue} /><Text style={styles.homeListingEmptyText}>No products match this filter.</Text></View>}
    </View>
...
function ProductDiscoverySearchPage({ products, favoriteIds, cartItems, initialQuery, onBack, onSearch, onFavorite, onAdd, onChangeCartQuantity, onOpen }: { products: Product[]; favoriteIds: Set<string>; cartItems: CartItem[]; initialQuery?: string; onBack: () => void; onSearch: (query: string) => void; onFavorite: (product: Product) => void; onAdd: (product: Product) => void; onChangeCartQuantity: (productId: string, change: number) => void; onOpen: (product: Product) => void }) {
  ...
  return <View style={styles.discoverySearchPage}>
    <View style={styles.discoverySearchHeader}><Pressable onPress={onBack} hitSlop={10} style={styles.discoverySearchBack}><Ionicons name="arrow-back" size={25} color={palette.heading} /></Pressable><TextInput autoFocus value={query} onChangeText={setQuery} onSubmitEditing={() => submit()} placeholder="Search products here" placeholderTextColor="#8A8F98" returnKeyType="search" style={styles.discoverySearchInput} /><Pressable onPress={() => submit()} hitSlop={8} style={styles.discoverySearchSubmit}><Ionicons name="search" size={22} color={palette.blue} /></Pressable></View>
    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" contentContainerStyle={styles.discoverySearchContent}>
      <Text style={styles.discoverySearchTitle}>Top Searches</Text>
      <View style={styles.discoverySearchChips}>{TOP_SEARCHES.map(term => <Pressable key={term} onPress={() => submit(term)} style={({ pressed }) => [styles.discoverySearchChip, pressed && styles.pressed]}><Ionicons name="trending-up" size={17} color={palette.white} /><Text style={styles.discoverySearchChipText}>{term}</Text></Pressable>)}</View>
      <Text style={[styles.discoverySearchTitle, styles.discoveryLovedTitle]}>People love these</Text>
      <View style={styles.discoveryProductGrid}>{products.slice(0, 6).map(product => <ProductCard key={`discovery-${product.id}`} item={product} width={cardWidth} favorite={favoriteIds.has(product.id)} quantity={cartItems.find(item => item.product.id === product.id)?.quantity ?? 0} collectionLayout onFavorite={() => onFavorite(product)} onAdd={() => onAdd(product)} onChangeQuantity={change => onChangeCartQuantity(product.id, change)} onOpen={() => onOpen(product)} />)}</View>
    </ScrollView>
  </View>;
}
...
function ProductDetail({ width, topInset, cartCount, cartQuantity, cartItems, product, recommendations, favoriteIds, onBack, onSearch, onAdd, onChangeCartQuantity, onCheckout, onOpenProduct, onFavorite }: { width: number; topInset: number; cartCount: number; cartQuantity: number; cartItems: CartItem[]; product: Product; recommendations: Product[]; favoriteIds: Set<string>; onBack: () => void; onSearch: () => void; onAdd: (product: Product) => void; onChangeCartQuantity: (productId: string, change: number) => void; onCheckout: () => void; onOpenProduct: (product: Product) => void; onFavorite: (product: Product) => void }) {
  const notifyConfirmation = useNotifyConfirmation();
  ...
  const floatingTotal = Number.isFinite(unitPrice) ? money(String(unitPrice), product.currencyCode ?? 'INR') : product.price;
...
function CartPopup({ visible, item, items, count, onOpen, containerStyle, footerProgress }: { visible: boolean; item: Product | null; items: CartItem[]; count: number; onOpen: () => void; containerStyle?: any; footerProgress?: Animated.Value }) {
  ...
  const imageProgress = useRef(new Animated.Value(1)).current;
  const popupWidth = useRef(new Animated.Value(items.length > 1 ? 184 : 158)).current;
  useEffect(() => {
    progress.stopAnimation();
    if (visible) {
      setDisplayCount(count);
      setDisplayProducts(items.slice(-2).reverse().map(entry => entry.product));
      setMounted(true);
      requestAnimationFrame(() => Animated.spring(progress, { toValue: 1, damping: 18, stiffness: 210, mass: 0.8, useNativeDriver: true }).start());
      return;
    }
    Animated.timing(progress, { toValue: 0, duration: 190, easing: Easing.inOut(Easing.cubic), useNativeDriver: true }).start(({ finished }) => { if (finished) setMounted(false); });
  }, [count, progress, visible]);
  useEffect(() => {
    if (!visible || !item) return;
    imageProgress.setValue(0);
    Animated.spring(imageProgress, { toValue: 1, damping: 17, stiffness: 230, mass: 0.7, useNativeDriver: true }).start();
  }, [imageProgress, item?.id, visible]);
  useEffect(() => {
    Animated.spring(popupWidth, {
      toValue: items.length > 1 ? 184 : 158,
      damping: 22,
      stiffness: 190,
      mass: 0.75,
  ...
  </Pressable></Animated.View></Animated.View>;
}
...
function HelpFab({ product, cartBottom = 0 }: { product?: Product | null; cartBottom?: number }) {
  ...
  const helpIconVisible = false;

  if (!helpIconVisible) return null;
...
function CartPage({ items, recentlyViewed, wishlistProducts, favoriteIds, onBack, onSearch, onChangeQuantity, onCheckout, onAddAddress = onCheckout, onOpenProduct, onAdd, onSeeAll, onOpenWishlist, onToggleFavorite }: { items: CartItem[]; recentlyViewed: Product[]; wishlistProducts: Product[]; favoriteIds: Set<string>; onBack: () => void; onSearch: () => void; onAddAddress?: () => void; onChangeQuantity: (productId: string, change: number) => void; onCheckout: () => void; onOpenProduct: (product: Product) => void; onAdd: (product: Product) => void; onSeeAll: () => void; onOpenWishlist: () => void; onToggleFavorite: (product: Product) => void }) {
  ...
  const cartOriginalTotal = items.reduce((total, { product, quantity }) => total + (Number((product.oldPrice || product.price).replace(/[^0-9.]/g, '')) || product.unitPrice || 0) * quantity, 0);
  const cartSavings = Math.max(0, cartOriginalTotal - cartTotal);
  const cartOriginalTotalText = money(String(cartOriginalTotal), items[0]?.product.currencyCode ?? 'INR');
  const cartSavingsText = money(String(cartSavings), items[0]?.product.currencyCode ?? 'INR');
  ...
  return <View style={styles.cartPage}>
    ...
    </View>)}</View>{recentlyViewed.length ? <View style={[styles.cartSuggestions, styles.cartContentBlock]}><Text style={styles.cartSuggestionsTitle}>You might also like</Text><View style={styles.cartSuggestionGrid}>{recentlyViewed.slice(0, 9).map(product => <ProductCard key={`cart-suggestion-${product.id}`} item={product} width={suggestionCardWidth} favorite={favoriteIds.has(product.id)} quantity={items.find(item => item.product.id === product.id)?.quantity ?? 0} collectionLayout onFavorite={() => onToggleFavorite(product)} onAdd={() => onAdd(product)} onChangeQuantity={change => onChangeQuantity(product.id, change)} onOpen={() => onOpenProduct(product)} />)}</View><Pressable onPress={onSeeAll} style={({ pressed }) => [styles.homeViewAllButton, styles.cartSeeAllButton, pressed && styles.pressed]}><View style={styles.homeViewAllThumbs}>{recentlyViewed.slice(0, 3).map((product, index) => <Image key={`cart-see-all-${product.id}`} source={product.image} style={[styles.homeViewAllThumb, index > 0 && styles.homeViewAllThumbOverlap]} resizeMode="contain" />)}</View><Text style={styles.homeViewAllText}>See all products</Text><Ionicons name="chevron-forward" size={16} color={palette.blue} /></Pressable></View> : null}{wishlistProducts.length ? <View style={[styles.cartWishlistBlock, styles.cartContentBlock]}><View style={styles.cartBlockHeader}><Text style={[styles.cartSuggestionsTitle, styles.cartBlockHeaderTitle]}>Your Wishlist</Text><Pressable onPress={onOpenWishlist} hitSlop={8}><Text style={styles.cartWishlistViewAll}>View all</Text></Pressable></View><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.cartWishlistRow}>{wishlistProducts.slice(0, 8).map(product => <ProductCard key={`cart-wishlist-${product.id}`} item={product} width={140} favorite quantity={items.find(item => item.product.id === product.id)?.quantity ?? 0} collectionLayout onFavorite={() => onToggleFavorite(product)} onAdd={() => onAdd(product)} onChangeQuantity={change => onChangeQuantity(product.id, change)} onOpen={() => onOpenProduct(product)} />)}</ScrollView></View> : null}<View style={[styles.cartOrderDetails, styles.cartContentBlock]}><Text style={styles.cartOrderDetailsTitle}>Order details</Text><View style={styles.cartOrderRow}><Text style={styles.cartOrderLabel}>Items MRP</Text><Text style={styles.cartOrderValue}>{cartOriginalTotalText}</Text></View><View style={styles.cartOrderRow}><Text style={styles.cartOrderLabel}>Product discount</Text><Text style={styles.cartOrderDiscount}>− {cartSavingsText}</Text></View><View style={styles.cartOrderRow}><Text style={styles.cartOrderLabel}>Shipping</Text><Text style={styles.cartOrderFree}>FREE</Text></View>{cartSavings > 0 ? <View style={styles.cartSavingsHighlight}><Ionicons name="sparkles-outline" size={16} color="#238636" /><Text style={styles.cartSavingsHighlightText}>You saved {cartSavingsText} on this order</Text></View> : null}<View style={styles.cartOrderDivider} /><View style={styles.cartOrderRow}><Text style={styles.cartOrderTotalLabel}>Total amount</Text><Text style={styles.cartOrderTotalValue}>{cartTotalText}</Text></View></View><View style={[styles.cartPoliciesBlock, styles.cartContentBlock]}><Text style={styles.cartOrderDetailsTitle}>Cancellation & returns</Text><View style={styles.cartPolicyRow}><View style={styles.cartPolicyIcon}><Ionicons name="close-circle-outline" size={20} color={palette.blue} /></View><View style={styles.cartPolicyCopy}><Text style={styles.cartPolicyTitle}>Cancellation policy</Text><Text style={styles.cartPolicyDescription}>You can cancel your order before it is shipped.</Text></View></View><View style={styles.cartPolicyDivider} /><View style={styles.cartPolicyRow}><View style={styles.cartPolicyIcon}><Ionicons name="return-down-back-outline" size={20} color={palette.blue} /></View><View style={styles.cartPolicyCopy}><Text style={styles.cartPolicyTitle}>Returns policy</Text><Text style={styles.cartPolicyDescription}>Eligible items can be returned within 7 days of delivery. Product conditions may apply.</Text></View></View></View></ScrollView> : <View style={styles.emptyCart}><Ionicons name="bag-handle-outline" size={44} color={palette.blue} /><Text style={styles.emptyCartTitle}>Your cart is empty</Text><Text style={styles.emptyCartCopy}>Add products to see them here.</Text></View>}
    {items.length ? <><Pressable onPress={() => setAddressSheetVisible(true)} style={styles.cartAddressBar}><LinearGradient pointerEvents="none" colors={['rgba(7,17,31,0)', 'rgba(7,17,31,0.12)']} style={styles.cartFooterTopShadow} /><View style={styles.cartAddressIcon}><Ionicons name="location-outline" size={20} color={palette.blue} /></View><View style={styles.cartAddressCopy}><Text style={styles.cartAddressTitle}>Add delivery address</Text><Text style={styles.cartAddressSubtitle}>Choose where you want your order delivered</Text></View><Text style={styles.cartAddressAction}>Add</Text><Ionicons name="chevron-forward" size={18} color={palette.blue} /></Pressable><View style={styles.cartCheckoutBar}><View style={styles.cartTotalBlock}><Text style={styles.cartItemCount}>{itemCount} {itemCount === 1 ? 'item' : 'items'} in cart</Text><Text style={styles.cartTotalText}>Total: {cartTotalText}</Text></View><Pressable onPress={onCheckout} style={styles.cartCheckoutButton}><Text style={styles.cartCheckoutText}>Place Order</Text></Pressable></View></> : null}
    <Modal visible={addressSheetVisible} transparent animationType="slide" onRequestClose={() => setAddressSheetVisible(false)}><Pressable style={styles.cartAddressBackdrop} onPress={() => setAddressSheetVisible(false)}><Pressable style={styles.cartAddressSheet} onPress={() => {}}><View style={styles.cartAddressHandle} /><View style={styles.cartAddressSheetHeader}><View><Text style={styles.cartAddressSheetTitle}>Add delivery location</Text><Text style={styles.cartAddressSheetSubtitle}>Choose how you would like to add your address</Text></View><Pressable onPress={() => setAddressSheetVisible(false)} style={styles.cartAddressClose}><Ionicons name="close" size={20} color={palette.heading} /></Pressable></View><Pressable onPress={() => { setAddressSheetVisible(false); onAddAddress(); }} style={styles.cartAddressOption}><View style={[styles.cartAddressOptionIcon, styles.cartAddressOptionIconPrimary]}><Ionicons name="add" size={24} color="#FFFFFF" /></View><View style={styles.cartAddressOptionCopy}><Text style={styles.cartAddressOptionTitle}>Add new address</Text><Text style={styles.cartAddressOptionSubtitle}>Enter your delivery details manually</Text></View><Ionicons name="chevron-forward" size={20} color="#8A94A3" /></Pressable><Pressable onPress={() => Alert.alert('Location access', 'Location-based address detection will be available soon. You can add an address manually for now.')} style={styles.cartAddressOption}><View style={styles.cartAddressOptionIcon}><Ionicons name="navigate-outline" size={22} color={palette.blue} /></View><View style={styles.cartAddressOptionCopy}><Text style={styles.cartAddressOptionTitle}>Use current location</Text><Text style={styles.cartAddressOptionSubtitle}>Allow location access to find your address</Text></View><Ionicons name="chevron-forward" size={20} color="#8A94A3" /></Pressable><Pressable onPress={() => { setAddressSheetVisible(false); onAddAddress(); }} style={styles.cartAddressOption}><View style={styles.cartAddressOptionIcon}><Ionicons name="keypad-outline" size={22} color={palette.blue} /></View><View style={styles.cartAddressOptionCopy}><Text style={styles.cartAddressOptionTitle}>Enter pincode</Text><Text style={styles.cartAddressOptionSubtitle}>Check delivery availability in your area</Text></View><Ionicons name="chevron-forward" size={20} color="#8A94A3" /></Pressable><View style={styles.cartNoSavedAddress}><Ionicons name="home-outline" size={21} color="#8A94A3" /><View><Text style={styles.cartNoSavedTitle}>No saved addresses yet</Text><Text style={styles.cartNoSavedCopy}>Your saved addresses will appear here.</Text></View></View></Pressable></Pressable></Modal>
  </View>;
}
...
function Storefront() {
  ...
  const [screen, setScreen] = useState<'home' | 'categories' | 'categoryCollection' | 'wishlist' | 'offers' | 'orders' | 'profile' | 'search' | 'discoverySearch' | 'homeProducts' | 'product' | 'cart' | 'checkout' | 'address' | 'orderSuccess' | 'orderFailure'>('home');
  ...
  const [openingAnimationVisible, setOpeningAnimationVisible] = useState(false);
  ...
  const footerScrollDirectionRef = useRef({ direction: 0, distance: 0 });
  ...
  const submitSearch = () => runSearch(searchQuery);
  ...
  useEffect(() => {
    ...
    footerScrollDirectionRef.current = { direction: 0, distance: 0 };
  ...
  useEffect(() => {
    if (!openingAnimationVisible) return;
  ...
  }, [contentWidth, openingAnimationVisible, openingFlightProgress, openingProgress, openingTruckProgress]);
  ...
  const addToCart = (product: Product) => {
    setCartItems(current => {
      ...
      return existing
        ? [...current.filter(item => item.product.id !== product.id), { ...existing, quantity: existing.quantity + 1 }]
  ...
  const openCart = () => {
    ...
    setCartReturnScreen(screen === 'product' || screen === 'categoryCollection' || screen === 'categories' || screen === 'wishlist' || screen === 'offers' || screen === 'orders' || screen === 'discoverySearch' || screen === 'homeProducts' ? screen : 'home');
    setScreen('cart');
  };
  ...
  const openProduct = (product: Product, preferredCollectionId?: string) => {
    ...
    if (screen !== 'product') setProductReturnScreen(screen === 'categoryCollection' || screen === 'categories' || screen === 'wishlist' || screen === 'offers' || screen === 'orders' || screen === 'search' || screen === 'discoverySearch' || screen === 'homeProducts' || screen === 'cart' ? screen : 'home');
  ...
  const renderProduct = (item: Product) => (
    <ProductCard key={item.id} item={item} width={cardWidth} favorite={favorites.has(item.id)} quantity={cartItems.find(cartItem => cartItem.product.id === item.id)?.quantity ?? 0} onFavorite={() => toggleFavorite(item)} onAdd={() => addToCart(item)} onChangeQuantity={change => changeCartQuantity(item.id, change)} onOpen={() => openProduct(item)} />
  );
  ...
  if (screen === 'categoryCollection' && selectedCategoryGroup && selectedCollectionItem) return <SafeAreaView style={[styles.safeArea, { backgroundColor: '#0A254A' }]}>
    ...
    <CategoryCollectionPage
      ...
      cartQuantities={Object.fromEntries(cartItems.map(item => [item.product.id, item.quantity]))}
      cartCount={cartCount}
      ...
      onChangeQuantity={(product, change) => changeCartQuantity(mapShopifyProduct(product).id, change)}
    ...
    <CartPopup visible={cartPopupVisible && cartCount > 0} item={cartPreview} items={cartItems} count={cartCount} onOpen={openCart} containerStyle={[styles.collectionCartPopupLayer, { paddingBottom: floatingCartBottom }]} />
    </Animated.View>
  </SafeAreaView>;

  if (screen === 'homeProducts') {
    const listingProducts = homeProductListing === 'trending' ? categoryProducts : bestSellingHomeProducts;
    return <SafeAreaView style={[styles.safeArea, { backgroundColor: '#0A254A' }]}><StatusBar barStyle="light-content" backgroundColor="#0A254A" translucent={false} /><Animated.View style={[styles.backRevealPage, { opacity: backRevealOpacity, transform: [{ translateX: backRevealTranslateX }] }]}><HomeProductListingPage title={homeProductListing === 'trending' ? 'Trending' : 'Best Selling'} products={listingProducts} favoriteIds={favorites} cartItems={cartItems} cartCount={cartCount} onBack={() => navigateBack('home')} onFavorite={toggleFavorite} onAdd={addToCart} onChangeCartQuantity={changeCartQuantity} onOpen={openProduct} onOpenSearch={() => openDiscoverySearch('homeProducts')} onOpenCart={openCart} onOpenWishlist={() => openFooterPage('wishlist')} /><CartPopup visible={cartPopupVisible && cartCount > 0} item={cartPreview} items={cartItems} count={cartCount} onOpen={openCart} containerStyle={{ paddingBottom: insets.bottom + 76 }} /></Animated.View></SafeAreaView>;
  }

  if (screen === 'discoverySearch') return <SafeAreaView style={[styles.safeArea, { backgroundColor: '#FFFFFF' }]}><StatusBar barStyle="dark-content" backgroundColor="#FFFFFF" translucent={false} /><Animated.View style={[styles.backRevealPage, { opacity: backRevealOpacity, transform: [{ translateX: backRevealTranslateX }] }]}><ProductDiscoverySearchPage products={bestSellingHomeProducts.length ? bestSellingHomeProducts : shopifyProducts} favoriteIds={favorites} cartItems={cartItems} initialQuery={discoverySearchInitialQuery} onBack={() => navigateBack(discoverySearchReturnScreen)} onSearch={runSearch} onFavorite={toggleFavorite} onAdd={addToCart} onChangeCartQuantity={changeCartQuantity} onOpen={openProduct} /><CartPopup visible={cartPopupVisible && cartCount > 0} item={cartPreview} items={cartItems} count={cartCount} onOpen={openCart} containerStyle={{ paddingBottom: insets.bottom + 12 }} /></Animated.View></SafeAreaView>;

  if (screen === 'product' && selectedProduct) {
    const recommendations = productPageRecommendations;
    return <SafeAreaView edges={['left', 'right', 'bottom']} style={[styles.safeArea, { backgroundColor: '#FFFFFF' }]}><StatusBar barStyle="dark-content" backgroundColor="transparent" translucent /><Animated.View style={[styles.backRevealPage, { opacity: backRevealOpacity, transform: [{ translateX: backRevealTranslateX }] }]}><ProductDetail key={selectedProduct.id} width={contentWidth} topInset={insets.top} cartCount={cartCount} cartQuantity={cartItems.find(item => item.product.id === selectedProduct.id)?.quantity ?? 0} cartItems={cartItems} product={selectedProduct} recommendations={recommendations} favoriteIds={favorites} onBack={() => navigateBack(productReturnScreen)} onSearch={() => openDiscoverySearch('product')} onAdd={addToCart} onChangeCartQuantity={changeCartQuantity} onCheckout={() => { setCartItems(current => current.length ? current : [{ product: selectedProduct, quantity: 1 }]); setCheckoutInitialStage(2); if (customerAuth.isLoggedIn) setScreen('address'); else setCheckoutLoginRequired(true); }} onOpenProduct={openProduct} onFavorite={toggleFavorite} /><CartPopup visible={cartPopupVisible && cartCount > 0} item={cartPreview} items={cartItems} count={cartCount} onOpen={openCart} containerStyle={{ paddingBottom: 84 }} /><HelpFab product={selectedProduct} cartBottom={floatingCartBottom} /></Animated.View></SafeAreaView>;
  }
    ...
    <SafeAreaView style={[styles.safeArea, (screen === 'home' || screen === 'categories' || screen === 'wishlist' || screen === 'offers' || screen === 'orders') && styles.homeSafeArea, screen === 'home' && { backgroundColor: activeHomeHeroTheme.colors[0] }]}>
      ...
      <Animated.View style={[styles.app, activeFooterTab === 'home' && styles.homeEntranceBackground, { width: contentWidth, opacity: Animated.multiply(storefrontEntranceProgress, backRevealOpacity), transform: [{ translateX: backRevealTranslateX }] }]}>
        ...
        <Animated.ScrollView
          ...
          onScroll={collapseBrowseChrome || screen === 'home' ? event => {
            ...
            if (offsetY <= 4) {
              footerScrollDirectionRef.current = { direction: 0, distance: 0 };
              setBrowseFooterVisibility(false);
            } else if (direction) {
              const tracker = footerScrollDirectionRef.current;
              footerScrollDirectionRef.current = direction === tracker.direction
                ? { direction, distance: tracker.distance + Math.abs(movement) }
                : { direction, distance: Math.abs(movement) };
              if (footerScrollDirectionRef.current.distance >= 18) {
                setBrowseFooterVisibility(direction > 0);
                footerScrollDirectionRef.current.distance = 0;
              }
            }
          } : undefined}
        ...
        >
          ...
          <ScrollView horizontal nestedScrollEnabled showsHorizontalScrollIndicator={false} directionalLockEnabled bounces alwaysBounceHorizontal decelerationRate="normal" scrollEventThrottle={16} overScrollMode="never" contentContainerStyle={styles.homeProductHorizontalRow}>
            {categoryProducts.slice(0, 9).map(item => <ProductCard key={`explore-${activeHomeMenu.label}-${item.id}`} item={item} width={trendingCardWidth} favorite={favorites.has(item.id)} quantity={cartItems.find(cartItem => cartItem.product.id === item.id)?.quantity ?? 0} collectionLayout onFavorite={() => toggleFavorite(item)} onAdd={() => addToCart(item)} onChangeQuantity={change => changeCartQuantity(item.id, change)} onOpen={() => openProduct(item)} />)}
          </ScrollView>
          ...
          <ScrollView horizontal nestedScrollEnabled showsHorizontalScrollIndicator={false} directionalLockEnabled bounces alwaysBounceHorizontal decelerationRate="normal" scrollEventThrottle={16} overScrollMode="never" contentContainerStyle={styles.homeProductHorizontalRow}>
            {bestSellingHomeProducts.slice(0, 9).map(item => <ProductCard key={`grid-${activeHomeMenu.label}-${item.id}`} item={item} width={trendingCardWidth} favorite={favorites.has(item.id)} quantity={cartItems.find(cartItem => cartItem.product.id === item.id)?.quantity ?? 0} collectionLayout onFavorite={() => toggleFavorite(item)} onAdd={() => addToCart(item)} onChangeQuantity={change => changeCartQuantity(item.id, change)} onOpen={() => openProduct(item)} />)}
          </ScrollView>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.homeProductHorizontalRow}>
            {recentlyViewed.map(item => <ProductCard key={`recent-${item.id}`} item={item} width={trendingCardWidth} favorite={favorites.has(item.id)} quantity={cartItems.find(cartItem => cartItem.product.id === item.id)?.quantity ?? 0} collectionLayout onFavorite={() => toggleFavorite(item)} onAdd={() => addToCart(item)} onChangeQuantity={change => changeCartQuantity(item.id, change)} onOpen={() => openProduct(item)} />)}
          </ScrollView>
          </View> : null}
        ...
        <Animated.View style={[styles.floatingFooter, { opacity: homeFooterOpacity, transform: [{ translateY: homeFooterTranslateY }] }]}>
...
const styles = StyleSheet.create({
  ...
  bottomSafeFill: { position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 25, backgroundColor: '#FFFFFF' },
  ...
  searchControls: { minHeight: 64, paddingHorizontal: 14, paddingVertical: 10, flexDirection: 'row', alignItems: 'center', gap: 8, borderTopWidth: 1, borderTopColor: '#DDE2E8', backgroundColor: '#FFFFFF', shadowColor: '#000000', shadowOffset: { width: 0, height: -3 }, shadowOpacity: 0.08, shadowRadius: 7, elevation: 10 },
  ...
  homeListingHeaderCopyBlock: { flex: 1, minWidth: 0 },
  ...
  homeListingHeaderActions: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  homeListingHeaderAction: { position: 'relative', width: 36, height: 40, alignItems: 'center', justifyContent: 'center' },
  homeListingBadge: { position: 'absolute', top: -4, right: -4, minWidth: 17, height: 17, paddingHorizontal: 3, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.red },
  ...
  homeListingProductGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'flex-start', rowGap: 18, columnGap: 8 },
  ...
  discoverySearchPage: { flex: 1, backgroundColor: palette.white },
  discoverySearchHeader: { height: 48, marginHorizontal: 16, marginTop: 8, paddingHorizontal: 4, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#D9DDE3', backgroundColor: palette.white },
  discoverySearchBack: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' },
  discoverySearchInput: { flex: 1, minWidth: 0, height: 44, paddingVertical: 0, color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 14, fontWeight: '600' },
  discoverySearchSubmit: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' },
  discoverySearchContent: { paddingHorizontal: 16, paddingTop: 26, paddingBottom: 30 },
  discoverySearchTitle: { color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 22, lineHeight: 28, fontWeight: '900' },
  discoverySearchChips: { marginTop: 18, flexDirection: 'row', flexWrap: 'wrap', gap: 9 },
  ...
  discoveryProductGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'flex-start', rowGap: 18, columnGap: 8 },
  ...
  carouselProgressTrack: { width: 18, height: 4, overflow: 'hidden', borderRadius: 2, backgroundColor: '#D3DDED' },
  carouselProgressFill: { height: '100%', borderRadius: 2, backgroundColor: palette.blue },
  carouselProgressDot: { width: 4, height: 4, borderRadius: 2, backgroundColor: '#778BA8' },
  ...
  collectionQuantityAction: { width: 58, minWidth: 58, paddingHorizontal: 2, flexDirection: 'row', justifyContent: 'space-between' },
  cardQuantityTap: { minWidth: 17, height: '100%', alignItems: 'center', justifyContent: 'center' },
  cardQuantityText: { color: '#FFFFFF', fontFamily: 'Inter_400Regular', fontSize: 12, fontWeight: '900' },
  ...
  productCardQuantityButton: { paddingHorizontal: 5, flexDirection: 'row', justifyContent: 'space-between', backgroundColor: palette.blue },
  ...
  floatingFooter: { position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 20, height: 70, paddingHorizontal: 4, flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderTopWidth: 1, borderColor: '#D5DBE3' },
  ...
  cartPopup: { width: '100%', height: 52, paddingHorizontal: 7, borderRadius: 12, flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(63,114,229,0.9)', shadowColor: '#000000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.18, shadowRadius: 7, elevation: 7 },
  cartPopupImages: { minWidth: 40, flexDirection: 'row', alignItems: 'center' },
  cartPopupImage: { width: 38, height: 38, borderWidth: 1, borderColor: 'rgba(255,255,255,0.82)', borderRadius: 19, backgroundColor: '#FFFFFF' },
  cartPopupImageOverlap: { marginLeft: -12 },
  ...
  cartPageHeader: { height: 58, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderColor: '#E4E7EC' },
  ...
  cartList: { padding: 12, gap: 12, paddingBottom: 16 },
  ...
  cartOrderDiscount: { color: '#238636', fontFamily: 'Inter_400Regular', fontSize: 13, fontWeight: '700' },
  cartOrderFree: { color: '#238636', fontFamily: 'Inter_400Regular', fontSize: 12, fontWeight: '800' },
  cartSavingsHighlight: { minHeight: 38, marginTop: 9, paddingHorizontal: 10, borderRadius: 8, flexDirection: 'row', alignItems: 'center', gap: 7, backgroundColor: '#EAF7ED' },
  cartSavingsHighlightText: { flex: 1, color: '#238636', fontFamily: 'Inter_400Regular', fontSize: 12, fontWeight: '700' },
  ...
  cartPoliciesBlock: { marginTop: 0 },
  cartPolicyRow: { minHeight: 66, flexDirection: 'row', alignItems: 'center' },
  cartPolicyIcon: { width: 38, height: 38, borderRadius: 11, alignItems: 'center', justifyContent: 'center', backgroundColor: '#EEF3FF' },
  cartPolicyCopy: { flex: 1, marginLeft: 11 },
  cartPolicyTitle: { color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 13, fontWeight: '700' },
  cartPolicyDescription: { marginTop: 3, color: '#697386', fontFamily: 'Inter_400Regular', fontSize: 11, lineHeight: 16 },
  cartPolicyDivider: { height: 1, marginLeft: 49, backgroundColor: '#ECEFF3' },
  ...
  cartCheckoutBar: { height: 70, paddingHorizontal: 14, paddingVertical: 8, borderTopWidth: 0, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFFFFF' },
  cartAddressBar: { minHeight: 46, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', backgroundColor: '#F6F7FB', zIndex: 2 },
  cartFooterTopShadow: { position: 'absolute', left: 0, right: 0, top: -6, height: 6 },
  cartAddressIcon: { width: 28, height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: '#EEF3FF' },
  cartAddressCopy: { flex: 1, marginLeft: 10 },
  cartAddressTitle: { color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 13, fontWeight: '700' },
  cartAddressSubtitle: { marginTop: 2, color: '#7B8490', fontFamily: 'Inter_400Regular', fontSize: 10 },
  cartAddressAction: { marginRight: 2, color: palette.blue, fontFamily: 'Inter_400Regular', fontSize: 12, fontWeight: '800' },
  cartAddressBackdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(10,18,30,0.48)' },
  cartAddressSheet: { paddingHorizontal: 16, paddingTop: 10, paddingBottom: 24, borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: '#F7F8FB' },
  cartAddressHandle: { width: 38, height: 4, marginBottom: 15, borderRadius: 2, alignSelf: 'center', backgroundColor: '#C7CDD5' },
  cartAddressSheetHeader: { marginBottom: 16, flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'space-between' },
  cartAddressSheetTitle: { color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 19, fontWeight: '800' },
  cartAddressSheetSubtitle: { marginTop: 4, color: '#697386', fontFamily: 'Inter_400Regular', fontSize: 11 },
  cartAddressClose: { width: 34, height: 34, borderRadius: 17, alignItems: 'center', justifyContent: 'center', backgroundColor: '#FFFFFF' },
  cartAddressOption: { minHeight: 72, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: '#EEF0F3', flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF' },
  cartAddressOptionIcon: { width: 40, height: 40, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: '#EEF3FF' },
  cartAddressOptionIconPrimary: { backgroundColor: palette.blue },
  cartAddressOptionCopy: { flex: 1, marginHorizontal: 11 },
  cartAddressOptionTitle: { color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 14, fontWeight: '700' },
  cartAddressOptionSubtitle: { marginTop: 3, color: '#7B8490', fontFamily: 'Inter_400Regular', fontSize: 10 },
  cartNoSavedAddress: { minHeight: 72, marginTop: 14, paddingHorizontal: 14, borderRadius: 14, flexDirection: 'row', alignItems: 'center', gap: 11, backgroundColor: '#FFFFFF' },
  cartNoSavedTitle: { color: '#667085', fontFamily: 'Inter_400Regular', fontSize: 13, fontWeight: '700' },
  cartNoSavedCopy: { marginTop: 2, color: '#98A0AD', fontFamily: 'Inter_400Regular', fontSize: 10 },
  ...
  cartCheckoutButton: { width: '58%', height: 50, borderRadius: 11, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.red },
  cartCheckoutText: { color: '#FFFFFF', fontFamily: 'Inter_400Regular', fontSize: 17, fontWeight: '800' },
  ...
  detailTitle: { color: palette.heading, fontFamily: 'Inter_400Regular', fontSize: 17, lineHeight: 23, fontWeight: '700' },
  ...
  addLarge: { width: '34%', height: 48, borderRadius: 10, flexDirection: 'row', gap: 7, alignItems: 'center', justifyContent: 'center', backgroundColor: palette.blue },
  detailCartQuantity: { width: '34%', height: 48, borderRadius: 10, paddingHorizontal: 3, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: palette.blue },
