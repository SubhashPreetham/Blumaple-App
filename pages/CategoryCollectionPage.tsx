...
import { useNotificationToast } from '../src/NotificationToast';
...
export function CategoryCollectionPage({ category, selectedCollection, previews, products, brands, shippingOptions, filterDataLoading, loading, totalProducts, loadingMore, hasNextPage, favoriteIds, cartCount, onOpenSearch, onOpenWishlist, onOpenCart, onBack, onSelectCollection, onLoadMore, onApplyFilters, onAdd, onToggleFavorite, onOpenProduct }: Props) {
  ...
  const [showProductScrollHint, setShowProductScrollHint] = useState(true);
  ...
  const visibleProducts = useMemo(() => {
    ...
    return filtered;
  }, [products, appliedBrands, appliedShipping, sortMode]);
  ...
  return <View style={s.page}>
    <View style={s.header}>
      <Pressable accessibilityRole="button" accessibilityLabel="Go back" onPress={onBack} style={s.headerButton}><Ionicons name="arrow-back" size={25} color="#FFFFFF" /></Pressable>
      <View style={s.headerCopy}><Text numberOfLines={2} style={s.headerTitle}>{category.title.trim()}</Text><Text numberOfLines={2} style={s.headerSubtitle}>Shop from verified collections</Text></View>
      <View style={s.headerActions}>
        <Pressable accessibilityRole="button" accessibilityLabel="Open search" onPress={onOpenSearch} style={s.headerButton}><Ionicons name="search-outline" size={25} color="#FFFFFF" /></Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="Open wishlist" onPress={onOpenWishlist} style={s.headerButton}><Ionicons name={favoriteIds.size ? 'heart' : 'heart-outline'} size={25} color="#FFFFFF" />{favoriteIds.size ? <View style={s.headerBadge}><Text style={s.headerBadgeText}>{favoriteIds.size > 99 ? '99+' : favoriteIds.size}</Text></View> : null}</Pressable>
        <Pressable accessibilityRole="button" accessibilityLabel="Open cart" onPress={onOpenCart} style={s.headerButton}><Ionicons name="cart-outline" size={26} color="#FFFFFF" />{cartCount ? <View style={s.headerBadge}><Text style={s.headerBadgeText}>{cartCount > 99 ? '99+' : cartCount}</Text></View> : null}</Pressable>
      </View>
    </View>

    <View style={s.content}>
      <ScrollView showsVerticalScrollIndicator={false} style={s.collectionRail} bounces directionalLockEnabled decelerationRate="normal" scrollEventThrottle={16} overScrollMode="never" contentContainerStyle={s.collectionRailContent}>
      ...
      <View style={s.productsPanel}>
        <View style={s.controls}>
          <Pressable onPress={() => setFilterVisible(true)} style={[s.control, activeFilterCount > 0 && s.controlActive]}><Ionicons name="options-outline" size={16} color={activeFilterCount > 0 ? '#FFFFFF' : '#2C2D2E'} /><Text style={[s.controlText, activeFilterCount > 0 && s.controlTextActive]}>Filters{activeFilterCount ? ` (${activeFilterCount})` : ''}</Text><Ionicons name="chevron-down" size={14} color={activeFilterCount > 0 ? '#FFFFFF' : '#2C2D2E'} /></Pressable>
          <Pressable onPress={() => setSortVisible(true)} style={[s.control, s.sortControl]}><Ionicons name="swap-vertical" size={16} color="#2C2D2E" /><Text numberOfLines={1} style={s.controlText}>Sort</Text><Ionicons name="chevron-down" size={14} color="#2C2D2E" /></Pressable>
        </View>
        <ScrollView showsVerticalScrollIndicator persistentScrollbar indicatorStyle="black" bounces alwaysBounceVertical decelerationRate="normal" scrollEventThrottle={16} overScrollMode="auto" contentContainerStyle={s.productScroll} onScroll={event => { const { contentOffset, contentSize, layoutMeasurement } = event.nativeEvent; setShowProductScrollHint(contentOffset.y + layoutMeasurement.height < contentSize.height - 24); }}>
        ...
        {showProductScrollHint && (visibleProducts.length > 4 || hasNextPage) ? <View pointerEvents="none" style={s.productScrollHint}><Ionicons name="chevron-down" size={16} color="#3F72E5" /><Text style={s.productScrollHintText}>Scroll for more</Text></View> : null}
      </View>
    </View>
...
const s = StyleSheet.create({
  ...
  header: { minHeight: 76, paddingHorizontal: 8, paddingVertical: 8, flexDirection: 'row', alignItems: 'center', gap: 6, borderBottomWidth: 1, borderColor: '#294565', backgroundColor: '#0A254A' },
  headerButton: { position: 'relative', width: 36, height: 44, alignItems: 'center', justifyContent: 'center' },
  headerCopy: { flex: 1, minWidth: 0 },
  headerTitle: { color: '#FFFFFF', fontFamily: 'Inter_400Regular', fontSize: 16, lineHeight: 20, fontWeight: '900' },
  headerSubtitle: { marginTop: 3, color: '#B9D6FF', fontFamily: 'Inter_400Regular', fontSize: 10, lineHeight: 13, fontWeight: '700' },
  headerActions: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  headerBadge: { position: 'absolute', top: 1, right: 0, minWidth: 17, height: 17, paddingHorizontal: 4, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: '#D83434', borderWidth: 1, borderColor: '#0A254A' },
  headerBadgeText: { color: '#FFFFFF', fontSize: 8, fontWeight: '900' },
  ...
  collectionRail: { width: 88, flexGrow: 0, flexShrink: 0, backgroundColor: '#F7F9FC', borderRightWidth: StyleSheet.hairlineWidth, borderColor: '#E5EAF1' },
  ...
  productScroll: { paddingTop: 12, paddingHorizontal: 8, paddingBottom: 30 },
  productScrollHint: { position: 'absolute', left: '50%', bottom: 10, minWidth: 112, height: 34, marginLeft: -56, paddingHorizontal: 12, borderWidth: 1, borderColor: '#D4DEEC', borderRadius: 17, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, backgroundColor: 'rgba(255,255,255,0.96)', shadowColor: '#000000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.12, shadowRadius: 5, elevation: 5 },
  productScrollHintText: { color: '#3F72E5', fontFamily: 'Inter_400Regular', fontSize: 10, fontWeight: '800' },
  ...
  heart: { position: 'absolute', right: 4, top: 4, width: 28, height: 28, alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' },
  productName: { minHeight: 54, marginTop: 2, color: '#2C2D2E', fontFamily: 'Inter_400Regular', fontSize: 13, lineHeight: 18, fontWeight: '700' },
  priceRow: { minHeight: 24, marginTop: 4, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'baseline', columnGap: 4 },
