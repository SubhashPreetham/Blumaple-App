...
export function SkippableLoginPage({ loading, error, onLogin, onGoogleLogin, onSkip }: { loading: boolean; error: string | null; onLogin: (identifier?: string, password?: string) => void; onGoogleLogin: () => void; onSkip: () => void }) {
  ...
  return <KeyboardAvoidingView style={s.page} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
    ...
    <Pressable onPress={() => leaveLogin(onSkip)} hitSlop={12} style={s.skip}><Text style={s.skipText}>Skip</Text></Pressable>
