...
export function OrderResultPage({ success, orderId, items, address, paymentMethod, total, tax, codFee, onHome, onRetry }: Props) {
  ...
  const deliveryRange: [number, number] = items.some(item => item.product.vendor?.trim().toLowerCase() === 'usa warehouse') ? [10, 15] : [2, 4];
  const deliveryDate = (days: number) => { const date = new Date(); date.setDate(date.getDate() + days); const day = date.getDate(); const suffix = day % 100 >= 11 && day % 100 <= 13 ? 'th' : day % 10 === 1 ? 'st' : day % 10 === 2 ? 'nd' : day % 10 === 3 ? 'rd' : 'th'; return `${date.toLocaleString('en-US', { month: 'short' })} ${day}${suffix}`; };
  const estimatedDelivery = `${deliveryDate(deliveryRange[0])} - ${deliveryDate(deliveryRange[1])}`;
  const downloadInvoice = async () => {
    if (!orderId) return;
    const invoice = new File(Paths.cache, `Blumaple-Invoice-${orderId.replace('/', '-')}.txt`);
    const lines = ['BLUMAPLE INVOICE', `Order ID: ${orderId}`, '', 'Items:', ...items.map(item => `${item.quantity} x ${item.product.name} — ${money(priceOf(item.product.price) * item.quantity)}`), '', `Tax included: ${money(tax)}`, ...(codFee > 0 ? [`COD processing fee: ${money(codFee)}`] : []), `Total: ${money(total)}`, `Payment mode: ${paymentMethod === 'cod' ? 'Cash on Delivery' : 'Online payment'}`, `Estimated delivery: ${estimatedDelivery}`, '', 'Delivery address:', address ? `${address.name}\n${address.phone}${address.email ? `\n${address.email}` : ''}\n${address.line}\n${address.city} - ${address.pincode}` : 'Not available'];
    invoice.write(lines.join('\n'));
    if (await Sharing.isAvailableAsync()) await Sharing.shareAsync(invoice.uri, { mimeType: 'text/plain', dialogTitle: 'Download invoice' });
  ...
  return <View style={s.page}>
    ...
    {success ? <View style={s.actionRow}><Pressable onPress={downloadInvoice} style={[s.actionButton, s.invoiceButton]}><Text style={s.invoiceText}>Download invoice</Text></Pressable><Pressable onPress={onHome} style={[s.actionButton, s.continueButton]}><Text style={s.primaryText}>Continue shopping</Text></Pressable></View> : <Pressable onPress={onRetry} style={s.primary}><Text style={s.primaryText}>Try again</Text></Pressable>}
  </View>;
}
...
const s = StyleSheet.create({
  page:{flex:1,backgroundColor:'#FFF'},header:{height:70,paddingHorizontal:16,flexDirection:'row',alignItems:'center',justifyContent:'space-between',borderBottomWidth:1,borderColor:'#F0F0F0'},headerTitle:{color:'#3F72E5',fontSize:15,fontWeight:'900'},spacer:{width:25},content:{padding:18,paddingBottom:110},icon:{width:68,height:68,borderRadius:34,alignSelf:'center',alignItems:'center',justifyContent:'center',marginTop:9},successIcon:{backgroundColor:'#2E8B36'},failIcon:{backgroundColor:'#D83434'},heading:{marginTop:13,textAlign:'center',color:'#1A1C1D',fontSize:24,fontWeight:'900'},subtitle:{marginTop:6,textAlign:'center',color:'#667085',fontSize:13,lineHeight:19},orderId:{marginTop:18,padding:14,borderRadius:10,backgroundColor:'#EEF3FF',flexDirection:'row',alignItems:'center',justifyContent:'space-between'},orderIdLabel:{color:'#536071',fontSize:13,fontWeight:'700'},orderIdValue:{color:'#3F72E5',fontSize:15,fontWeight:'900'},section:{marginTop:22,marginBottom:9,color:'#1A1C1D',fontSize:18,fontWeight:'900'},card:{padding:14,borderWidth:1,borderColor:'#E4E7EC',borderRadius:12,backgroundColor:'#FFF'},item:{flexDirection:'row',alignItems:'center'},image:{width:48,height:48,borderRadius:6},itemCopy:{flex:1,marginLeft:9,paddingRight:8},product:{color:'#1A1C1D',fontSize:13,fontWeight:'800',lineHeight:18},quantity:{marginTop:3,color:'#697386',fontSize:11},itemPrice:{color:'#1A1C1D',fontSize:13,fontWeight:'900'},line:{height:1,marginVertical:12,backgroundColor:'#E7E9EC'},smallGap:{height:8},row:{flexDirection:'row',justifyContent:'space-between'},rowLabel:{color:'#536071',fontSize:13},rowValue:{color:'#1A1C1D',fontSize:13,fontWeight:'800'},total:{color:'#1A1C1D',fontSize:16,fontWeight:'900'},payment:{color:'#1A1C1D',fontSize:14,fontWeight:'800'},customer:{color:'#1A1C1D',fontSize:15,fontWeight:'900',marginBottom:4},address:{color:'#536071',fontSize:13,lineHeight:19},primary:{position:'absolute',left:0,right:0,bottom:0,height:78,paddingHorizontal:16,borderTopWidth:1,borderColor:'#E5E5E5',alignItems:'center',justifyContent:'center',backgroundColor:'#FFF'},actionRow:{position:'absolute',left:0,right:0,bottom:0,height:78,paddingHorizontal:16,flexDirection:'row',alignItems:'center',gap:10,borderTopWidth:1,borderColor:'#E5E5E5',backgroundColor:'#FFF'},actionButton:{flex:1,height:54,borderRadius:10,alignItems:'center',justifyContent:'center'},continueButton:{backgroundColor:'#3F72E5'},invoiceButton:{borderWidth:1.5,borderColor:'#3F72E5',backgroundColor:'#FFF'},primaryText:{color:'#FFF',fontSize:13,fontWeight:'900'},invoiceText:{color:'#3F72E5',fontSize:13,fontWeight:'900'},
});
